...
const ProfilePost: React.FC<Props> = ({ post, isFavorite, deletePost, setEditingPost, removeFromFavorites }) => {
        ...
        if (isFavorite) {
            return (
                <button type="button" className="btn btn-lg p-0 pr-2" onClick={removeFromFavoriteHandler}>
                    <i className="fa fa-trash p-0 text-danger"></i>
                </button>
            );
        }
        return (
            <>
                <button type="button" className="btn btn-lg p-0" onClick={editHandler}>
                    <i className="fa fa-edit p-0 text-info"></i>
                </button>
    ...
    }, [isFavorite, removeHandler, editHandler, removeFromFavoriteHandler]);

    return (
        <>
            <div className={`${s.card} card my-2 w-100`}>
                <div className="d-md-flex border no-gutters">
                    <img className={`card-img border-right ${s.card_img} col-md-2`} src={post.image ? post.image : "https://kknd26.ru/images/no_photo.png"} alt="foto" />
                    <div className="w-100">
