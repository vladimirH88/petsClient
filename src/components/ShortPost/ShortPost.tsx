...
const ShortPost: React.FC<Props> = ({ post, isFavorite, isAuthorized, addToFavorites, removeFromFavorites }) => {
	const { t } = useTranslation();
	const [favorite, setFavorite] = useState(isFavorite);

	const favoriteHandler = useCallback((e: React.MouseEvent) => {
		e.preventDefault();
		if (!isAuthorized) {
		...
		setFavorite(true);
	}, [isAuthorized, favorite, setFavorite, addToFavorites, removeFromFavorites, post.id]);
	...
	return (
		<div className={`${s.card} card my-3 w-100`}>
			<div className="d-md-flex border no-gutters">
				...
				<img className={`card-img ${s.card_img} col-md-2 border-right`} src="https://kknd26.ru/images/no_photo.png" alt="foto" />
				<div className="w-100">
					<div className="d-flex justify-content-between align-items-center border-bottom">
						<h6 className={`${s.briefDescription} ml-3 mb-0`}>
							{post.briefDescription}
						</h6>
						<h6 className="price mr-3  mb-0">
							{post.price ? `${t('post.price', { price: post.price })}` : `${t('post.free')}`}
							<i className={`fa fa-star py pr-0 ml-2 ${isFavorite ? 'text-success' : ''}`} onClick={favoriteHandler}></i>
						</h6>
					</div>
					<div className="d-md-flex justify-content-between">
						...
						<div className={`${s.card_text}`}>
							<p className={`card-text mx-3 float-right ${s.card_text}`}>
								{post.description}
							</p>
						</div>
					</div>
